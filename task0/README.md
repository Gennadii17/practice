...
> Если папка src отсутствует, то создадим её вручную:

> ```
> Правой кнопкой мыши на папке task0 => 
> New => Directory => Ввести имя src
> ```
> Обрати внимание, что папка подсвечена синим - это означает, что IntelliJ IDEA пометила её как источник всех *.java файлов


1. В папке `src` создать пакет по такому шаблону:
`(имя/никнейм).task0`
Должно получится `test.task0`

> Пакеты создаются так же как и обычные директории:

> ```
> Правой кнопкой мыши на папке task0 => 
> New => Package => Ввести имя пакета
> ```

2. Создадим точку входа в приложение.

> Классы создаются так же как и обычные директории:

> ```
> Правой кнопкой мыши на пакете `(имя/никнейм).task0` => 
> New => Java Class => Ввести имя класса
> ```

> Не забудь про `public static void main(String[] args)`!

3. Сделаем build нашего проекта

> Справа вверху, есть кнопка с зелёным молотком или это можно сделать с помощью Ctrl + F9

По окончанию процесса, рядом с папкой `src` должна появится новая папка с названием `out`.
Там ты должен увидеть *.class - твои скомпилированные классы.

4. Запустим проект

> Рядом с `public static void main(String[] args)` слева есть зелёная кнопка Play. Она запустит наше приложение.
> Обязательно попробуй запустить приложение с помощью `java`.
```
Внизу открой Terminal =>
Введи cd out\production\task0\(имя/никнейм)\task0 =>
После этого введи java * - где * это имя твоего класса, точки входа в приложение
```
> По факту, кнопка Play делает то же самое что и команда `java *`, просто нам не нужно вводить другие инструкции или подсказки для поиска этих файлов.

5. Сделать pull проекта в репозиторий

```
VCS => Commit...
Выбрать файл *.java => В окошке Commit Message ввести фразу "Task is finished" =>
Нажать Commit

VCS => Git => Push... => Нажать кнопку Push
```

Потом перейти по этой [ссылке](https://github.com/Gennadii17/practice/tree/master/task0) и удостовериться, что изменения попали в удалённое хранилище.

После этого создать Pull Request (На сайте ГитХаба перейти в https://github.com/Gennadii17/practice/pulls и нажать на зеленую кнопку New Pull Request)
